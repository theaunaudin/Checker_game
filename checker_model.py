...
class CheckerModel:
	...
	def ia_move(self, model, depth_minimax = None, to_maximise = False, number_of_games = 1000, depth = 3, agent_turn = 1):
		if model == "random": 
			selected_piece_position, move_position = self.random_model_predict()
			self.move_piece(selected_piece_position, move_position)
			
		elif model == "minimax":
			best_selected_piece_position, best_move_position = self.minimax_model_predict(depth_minimax, to_maximise)
			self.move_piece(best_selected_piece_position, best_move_position)
		
		elif model == "monte_carlo":
			best_selected_piece_position, best_move_position = self.monte_carlo_model_predict(number_of_games,depth,agent_turn)
			self.move_piece(best_selected_piece_position, best_move_position)
	...
	def monte_carlo_model_predict(self,number_of_games,depth,agent_turn):
		...
		best_score = float("inf") if agent_turn==1 else -float("inf")
		best_selected_piece_position, best_move_position = None, None
	...
	def monte_carlo(self, number_of_games ,depth, agent_turn):
		sum_score=0
		for number_of_game_index in range(number_of_games):
			winedown=depth
